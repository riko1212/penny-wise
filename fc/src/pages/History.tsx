...
interface HistoryPoint { period: string; total: number; }
interface CatHistoryPoint { period: string; categoryName: string; total: number; }
...
export default function History() {
  ...
  const [catData, setCatData] = useState<CatHistoryPoint[]>([]);
  ...
  const getTotal = (period: string | number): number => data.find((d) => d.period === String(period))?.total ?? 0;
  ...
  function getCatTotal(period: string, cat: string): number {
    return catData.find((d) => d.period === period && d.categoryName === cat)?.total ?? 0;
  }

  const stackedData: Record<string, string | number>[] = [...new Set(catData.map((d) => d.period))].sort().map((period) => {
