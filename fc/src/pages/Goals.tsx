...
const EMPTY_FORM: GoalFormData = { name: '', targetAmount: '', categoryName: '', note: '', dueDate: '' };
...
export default function Goals() {
  ...
  function dueDateInfo(dueDate: number | null | undefined, reached: boolean): { label: string; overdue: boolean } | null {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      if (editingGoal) {
        ...
        const updated = await apiFetch(`/api/goals?userId=${currentUser.id}`).then((r) => r.json()) as Goal[];
        setGoals(updated);
      } else {
        ...
        if (!res.ok) throw new Error(await res.text());
        const refreshed = await apiFetch(`/api/goals?userId=${currentUser.id}`).then((r) => r.json()) as Goal[];
        setGoals(refreshed);
      }
      ...
      showToast('error', message || t('goals.failedSave'));
    } finally {
      setSaving(false);
    }
  }
