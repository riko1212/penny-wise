...
public class TransactionController {
    ...
    // Get per-category summary by user and type (optional month/year filter)
    @GetMapping("/summary")
    public ResponseEntity<List<java.util.Map<String, Object>>> getSummary(
            ...
            @RequestParam(required = false) Integer month,
            @RequestParam(required = false) Integer year) {
        return ResponseEntity.ok(transactionService.getSummaryByType(userId, type, month, year));
    }
